...
class Auto_ProxyAnchor(torch.nn.Module):
    def __init__(self, nb_classes, sz_embedding, mgr = 0.1, alpha = 32, ):
    ...
    def forward(self, X, T):
        ...
        pos_exp = torch.exp(-self.alpha * (cos - self.mgr))
        neg_exp = torch.exp(self.alpha * (cos + self.mgr))
...
#    def __init__(self, nb_classes, sz_embedding, scale=32):
#        super(ProxyNCA, self).__init__()
#        self.nb_classes = nb_classes
#        self.sz_embedding = sz_embedding
#        self.scale = scale
#        self.loss_func = losses.ProxyNCALoss(num_classes = self.nb_classes, embedding_size = self.sz_embedding, softmax_scale = self.scale).cuda()
...
class ProxyNCA(torch.nn.Module):
    def __init__(self, nb_classes, sz_embedding, smoothing_const=None, scaling_x=None, scaling_p=None):
        super(ProxyNCA, self).__init__()
        self.num_proxies = nb_classes
        self.embedding_dim = sz_embedding
